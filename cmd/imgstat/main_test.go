...
func TestReportRejectsNegativeN(t *testing.T) {
	out, err := captureStdout(func() error {
		return report("test", bytes.NewReader(nil), -1, 0, false, false)
	})
	if err == nil {
...
func TestReportRejectsNegativeK(t *testing.T) {
	out, err := captureStdout(func() error {
		return report("test", bytes.NewReader(nil), 0, -1, false, false)
	})
	if err == nil {
...
func TestReportPaletteHeaderUsesActualClusterCount(t *testing.T) {
	...
	out, err := captureStdout(func() error {
		return report("test.png", bytes.NewReader(buf.Bytes()), defaultTop, defaultK, false, false)
	})
	if err != nil {
